...
type AddPaisFormProps = {
    onAddSuccess: (newPais: Pais) => void
    onCancel: () => void
}
...
const AddPaisForm: React.FC<AddPaisFormProps> = ({
    onAddSuccess,
    onCancel,
}) => {
    ...
    const handleSubmit = async (values: Partial<Pais>) => {
        try {
            ...
            const newPais = await createPais(values)
            Swal.fire({
                title: 'Éxito',
    ...
    const handleDocumentAdd = () => {
        if (
            tempUniversity.trim() &&
            !selectedDocuments.includes(tempUniversity.trim())
        ) {
            setSelectedDocuments([...selectedDocuments, tempUniversity.trim()])
            setTempUniversity('')
        }
    }
